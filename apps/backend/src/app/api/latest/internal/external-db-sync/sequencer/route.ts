...
async function backfillSequenceIds(batchSize: number): Promise<boolean> {
  return await traceSpan({
    ...
    if (projectUserTenants.length > 0) {
      const distinctTenancyIds = [...new Set(projectUserTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (contactChannelTenants.length > 0) {
      const distinctTenancyIds = [...new Set(contactChannelTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (teamTenants.length > 0) {
      const distinctTenancyIds = [...new Set(teamTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
      ...
      await globalPrismaClient.$executeRaw`
        ...
        FROM (
          ...
          WHERE "Team"."tenancyId" IN (${Prisma.join([...new Set(teamTenants.map(t => t.tenancyId))])})
    ...
    if (teamMemberTenants.length > 0) {
      const distinctTenancyIds = [...new Set(teamMemberTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (teamPermissionTenants.length > 0) {
      const distinctTenancyIds = [...new Set(teamPermissionTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    const teamInvitationTenants = await globalPrismaClient.$queryRaw<{ tenancyId: string }[]>`
      ...
      updated_rows AS (
        ...
        WHERE vc."projectId" = r."projectId"
          ...
          AND vc."id"        = r."id"
        RETURNING vc."projectId", vc."branchId", vc."data"
      )
      SELECT DISTINCT "Team"."tenancyId"
      FROM updated_rows
      JOIN "Team" ON "Team"."teamId" = (updated_rows."data"->>'team_id')::uuid
    `;
    ...
    if (teamInvitationTenants.length > 0) {
      const distinctTenancyIds = [...new Set(teamInvitationTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (emailOutboxTenants.length > 0) {
      const distinctTenancyIds = [...new Set(emailOutboxTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (projectPermissionTenants.length > 0) {
      const distinctTenancyIds = [...new Set(projectPermissionTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (notificationPreferenceTenants.length > 0) {
      const distinctTenancyIds = [...new Set(notificationPreferenceTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (refreshTokenTenants.length > 0) {
      const distinctTenancyIds = [...new Set(refreshTokenTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (oauthAccountTenants.length > 0) {
      const distinctTenancyIds = [...new Set(oauthAccountTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
    ...
    if (deletedRowTenants.length > 0) {
      const distinctTenancyIds = [...new Set(deletedRowTenants.map(t => t.tenancyId))];
      await enqueueExternalDbSyncBatch(distinctTenancyIds);
      didUpdate = true;
    }
