...
let lastLocalConnectionErrorWarnTime = 0;
...
function isLocalConnectionError(error: unknown): boolean {
  ...
  if (err && (err.code === "ECONNREFUSED" || err.cause?.code === "ECONNREFUSED")) {
    return true;
  }
  return false;
}

function warnLocalConnectionError() {
  const now = performance.now();
  if (now - lastLocalConnectionErrorWarnTime > WARN_INTERVAL_MS) {
    console.warn(
      `[Poller] QStash service is unreachable at "${getEnvVariable("STACK_QSTASH_URL", "")}". ` +
      `Make sure your Docker environment / local QStash container is running if you are testing external database sync/webhooks.`
    );
    lastLocalConnectionErrorWarnTime = now;
  }
}
    ...
    return await traceSpan("external-db-sync.poller", async (span) => {
      ...
      while (performance.now() - startTime < maxDurationMs) {
        const iterationResult = await traceSpan<PollerIterationResult>({
          ...
          const staleRequests = await globalPrismaClient.$queryRaw<{ id: string, startedFulfillingAt: Date, deduplicationKey: string | null }[]>`
            SELECT "id", "startedFulfillingAt", "deduplicationKey"
            FROM "OutgoingRequest"
          ...
          if (staleRequests.length > 0) {
            for (const req of staleRequests) {
              try {
                ...
                // If it violates the partial unique constraint on deduplicationKey,
                // another pending request with the same key already exists.
                if (e?.message?.includes('duplicate key') || e?.meta?.driverAdapterError?.cause?.originalCode === '23505') {
                  await globalPrismaClient.$executeRaw`
                    DELETE FROM "OutgoingRequest" WHERE "id" = ${req.id}::uuid
                  `;
                } else {
                  captureError("poller-stale-outgoing-request-reset-error", e);
                }
              }
            }
            if (getNodeEnvironment().includes("development") || getNodeEnvironment().includes("test")) {
              console.warn(
                `[Poller] Found and handled ${staleRequests.length} outgoing request(s) with startedFulfillingAt older than ${STALE_REQUEST_THRESHOLD_MS}ms. ` +
                `These requests likely failed because the QStash service is unreachable.`
              );
            } else {
              captureError(
                "poller-stale-outgoing-requests",
                new StackAssertionError(
                  `Found and handled ${staleRequests.length} outgoing request(s) with startedFulfillingAt older than ${STALE_REQUEST_THRESHOLD_MS}ms`,
