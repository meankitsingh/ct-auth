import { GetObjectCommand, PutObjectCommand, S3Client, ListObjectsV2Command, DeleteObjectsCommand } from "@aws-sdk/client-s3";
...
export async function clearS3Buckets() {
  if (!s3Client) return;

  const buckets = [S3_BUCKET, S3_PRIVATE_BUCKET].filter(Boolean);
  for (const bucket of buckets) {
    try {
      ...
      console.log(`[S3] Emptied bucket: ${bucket}`);
    } catch (err) {
      console.warn(`[S3] Failed to empty bucket ${bucket}:`, err);
    }
  }
}
