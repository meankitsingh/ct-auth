...
const dropSchema = async () => {
  ...
  // Clear S3 storage buckets
  try {
    await clearS3Buckets();
  } catch (err) {
    console.warn("Failed to clear S3 mock buckets:", err);
  }
  ...
  // Drop managed views, row policies, and limited_user in default database to completely reset ClickHouse
  const tables = [
  ...
  console.log("[Clickhouse] Dropping old default views, policies, and users...");
  await Promise.all(tables.map(table =>
    clickhouseClient.command({ query: `DROP VIEW IF EXISTS default.${table}` }).catch(() => {})
  ));
  await Promise.all(tables.map(table =>
    clickhouseClient.command({ query: `DROP ROW POLICY IF EXISTS ${table}_project_isolation ON default.${table}` }).catch(() => {})
  ));
  await clickhouseClient.command({ query: "DROP USER IF EXISTS limited_user" }).catch(() => {});
};
