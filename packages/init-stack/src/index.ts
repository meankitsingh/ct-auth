...
const nextSteps: string[] = [
  `Create an account and Stack Auth API key for your project on https://accounts.cognitiontree.com`,
];
...
1. ${noBrowser ?
      `Create a project at https://accounts.cognitiontree.com and get your API keys` :
...
const Steps = {
  ...
  async writeEnvVars(type: "js" | "next" | "react"): Promise<boolean> {
    ...
    if (potentialEnvLocations.every((p) => !fs.existsSync(p))) {
      const envContent = noBrowser
        ...
        "# 1. Go to https://accounts.cognitiontree.com\n" +
        ...
        : "# Stack Auth keys\n" +
        "# Get these variables by creating a project on https://accounts.cognitiontree.com.\n" +
        `NEXT_PUBLIC_STACK_PROJECT_ID="${projectIdFromArgs ?? ""}"\n` +
        `NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY="${publishableClientKeyFromArgs ?? ""}"\n` +
        "STACK_SECRET_SERVER_KEY=\n";
  ...
  async writeStackAppFile({ type, srcPath, defaultExtension, indentation }: StackAppFileOptions, clientOrServer: "server" | "client", alsoHasClient: boolean): Promise<StackAppFileResult> {
    ...
    const jsOptions = type === "js" ? [
      `\n\n${indentation}// get your Stack Auth API keys from https://accounts.cognitiontree.com${clientOrServer === "client" ? ` and store them in a safe place (e.g. environment variables)` : ""}`,
