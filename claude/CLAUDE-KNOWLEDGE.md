...
Q: What does the error `PrismaClientKnownRequestError: Can't reach database server at 127.0.0.1:8128` mean and how can it be resolved?
A: This error indicates that the PostgreSQL database container (or Docker itself) is not running or is unreachable. Inside a running container, `127.0.0.1` refers to the container itself rather than the host machine. To resolve this, ensure Docker Desktop is running and start dependency containers via `pnpm start-deps`. We also changed all hardcoded environment variables (both build-time and runtime fallbacks) in the `Dockerfile`s to use the `ARG` + `ENV` pattern. This allows them to be completely overridden at build time via `--build-arg` while defaulting to safe local-development values (like `host.docker.internal:8128` for the database connection) so it runs directly out-of-the-box.